...
class Dense:
    ...
    def backward(self, x):
        self.delta = x.reshape(1, -1)
        delta = np.dot(self.delta, self.weight)
        return delta
    ...
    def adam(self, lr):
        ...
        self.bv = beta2 * self.bv + (1.0 - beta2) * self.d_bias * self.d_bias
        ...
        self.wv = beta2 * self.wv + \
            (1.0 - beta2) * self.d_weight * self.d_weight
